...
app.get('/status-info', (req, res) => {
  ...
  if (statusCode[code]) {
    res.status(code).json({ status: code, message: statusCode[code] });
  } else {
    res.status(400).json({ status: 400, message: "Invalid status code provided. Please provide a valid HTTP status code." });
  }
});
